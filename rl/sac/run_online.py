...
def run(args):
    ...
    if cfg.load_path:
        load_path = script_dir + f"/{cfg.load_path}/{cfg.env.env_name}/{cfg.seed}/step_{cfg.load_step}"
        agent.load(load_path)
        buffer.load(load_path)
        print(f"Successfully loaded from {load_path}")
    start_step = cfg.load_step if cfg.load_path else 0

    # initial evaluation
    eval_info = evaluate(agent, eval_env, cfg.num_eval_episodes)
    logger.update_metric(**eval_info)
    logger.log_metric(step=start_step * cfg.action_repeat * cfg.num_train_envs)
    logger.reset()
    ...
    for interaction_step in tqdm.tqdm(
        range(start_step+1, int(cfg.num_interaction_steps+1)), smoothing=0.1
    ):
        # collect data
        ...
        if timestep is None or buffer.can_sample() is False:
        ...
        if buffer.can_sample():
            ...
            if interaction_step % cfg.save_checkpoint_per_interaction_step == 0:
                step_save_path = save_path + f'/step_{interaction_step}'
                agent.save(step_save_path)
                buffer.save(step_save_path)
