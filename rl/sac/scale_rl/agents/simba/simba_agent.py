...
class SimbaConfig:
    ...
    ns_steps: int
    reinit_on_load: bool
    # AIPR: Adaptive Isometric Policy Regularization parameters
    aipr_lambda_0: float = 0.01      # base regularization strength
    aipr_tau: float = 1.0            # DfI activation threshold
    aipr_alpha: float = 0.5          # sigmoid temperature
...
def _update_simba_networks(
    ...
    if critic_use_categorical:
        new_actor, actor_info = update_actor_with_categorical_critic(
            ...
            aipr_lambda_0=aipr_lambda_0,
            aipr_tau=aipr_tau,
            aipr_alpha=aipr_alpha,
        )
    else:
...
class SimbaAgent(BaseAgent):
    ...
    def update(self, update_step: int, batch: Dict[str, np.ndarray]) -> Dict:
        ...
        (
            ...
            aipr_lambda_0=self._cfg.aipr_lambda_0,
            aipr_tau=self._cfg.aipr_tau,
            aipr_alpha=self._cfg.aipr_alpha,
        )
        ...
        # AIPR replaces discrete reinit: continuous regularization handles plasticity.
        # Discrete reinit is still supported as an optional complement (set reinit_update_steps=[]).
        if update_step in self._cfg.reinit_update_steps:
            if self._cfg.reinit_type == 'full_reset':
                self._init_network()
            elif self._cfg.reinit_type == 'aipr':
                self._orthogonally_project_network()
            elif self._cfg.reinit_type == 'snp':
                self._snp_network()
            elif self._cfg.reinit_type == 'aipr':
                pass  # AIPR is always-on continuous regularization; no discrete action needed
            else:
                raise ValueError
    ...
    def load(self, path: str) -> None:
        ...
        if self._cfg.reinit_on_load:
            ...
            if self._cfg.reinit_type == 'full_reset':
                ...
                pass  # AIPR handles plasticity continuously; no discrete action on load
            else:
                raise ValueError
