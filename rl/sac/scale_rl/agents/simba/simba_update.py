...
from scale_rl.agents.simba.aipr_regularizer import compute_aipr_loss
...
def update_actor(
    ...
    def actor_loss_fn(
        ...
        rl_loss = (log_probs * temperature() - q).mean()

        # AIPR: adaptive isometric regularization (continuous alternative to AIPR reinit)
        aipr_loss = compute_aipr_loss(actor_params, aipr_lambda_0, aipr_tau, aipr_alpha)
        actor_loss = rl_loss + aipr_loss

        actor_info = {
            "actor/loss": actor_loss,
            "actor/rl_loss": rl_loss,
            "actor/aipr_loss": aipr_loss,
...
def update_actor_with_categorical_critic(
    ...
    def actor_loss_fn(
        ...
        # AIPR: adaptive isometric regularization
        aipr_loss = compute_aipr_loss(actor_params, aipr_lambda_0, aipr_tau, aipr_alpha)
        actor_loss = rl_loss + aipr_loss
...
def update_critic(
    ...
    def critic_loss_fn(
        ...
        if critic_use_cdq:
            ...
            # compute mse loss
            td_loss = ((pred_q1 - target_q) ** 2 + (pred_q2 - target_q) ** 2).mean()
        else:
            ...
            # compute mse loss
            td_loss = ((pred_q - target_q) ** 2).mean()

        # AIPR: adaptive isometric regularization for critic
        aipr_loss = compute_aipr_loss(critic_params, aipr_lambda_0, aipr_tau, aipr_alpha)
        critic_loss = td_loss + aipr_loss

        critic_info = {
            "critic/loss": critic_loss,
            "critic/td_loss": td_loss,
            "critic/aipr_loss": aipr_loss,
...
def update_categorical_critic(
    ...
    def critic_loss_fn(
        ...
        # AIPR: adaptive isometric regularization for categorical critic
        aipr_loss = compute_aipr_loss(critic_params, aipr_lambda_0, aipr_tau, aipr_alpha)
        td_loss = critic_loss
        critic_loss = td_loss + aipr_loss
