...
class NpyUniformBuffer(BaseBuffer):
    ...
    def load(self, path: str) -> None:
        file_path = os.path.join(path, "dataset.pickle")
        assert os.path.exists(file_path)

        with open(file_path, "rb") as f:
            dataset = pickle.load(f)
        num_loaded = len(dataset["observation"])

        self._observations[:num_loaded] = dataset["observation"][:num_loaded]
        self._actions[:num_loaded] = dataset["action"][:num_loaded]
        self._rewards[:num_loaded] = dataset["reward"][:num_loaded]
        self._terminateds[:num_loaded] = dataset["terminated"][:num_loaded]
        self._truncateds[:num_loaded] = dataset["truncated"][:num_loaded]
        self._next_observations[:num_loaded] = dataset["next_observation"][:num_loaded]

        self._num_in_buffer = num_loaded
        self._current_idx = num_loaded % self._max_length
        self._n_step_transitions.clear()
        
        print(f"Successfully loaded {self._num_in_buffer} transitions.")
